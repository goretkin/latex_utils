...
# GG provenance: https://gist.github.com/brechtm/891de9f72516c1b2cbc1
...
function diff_page {
    ...
    convert - \( -clone 0-1 -compose darken -composite \) \
            -channel RGB -combine $DIFFDIR/$page_number.$OUTEXT
...
#            -highlight-color blue pdfdiff/$page_number.png
    ...
    grayscale=$(convert pdfdiff/$page_number.$OUTEXT -colorspace HSL -channel g -separate +channel -format "%[fx:mean]" info:)
    if [ "$grayscale" != "0" ]; then
